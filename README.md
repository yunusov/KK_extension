...
|---|---|
| Корень карточки | `.ContentStyles__Card-sc-19y55e6-0` |
| Название заказа | цепочка `children[1][0][0][0]` корневой карточки |
| Дата создания | первый `.order-card-param-subitem-list__element > p` (текст вида `Заказ оставлен 4 октября в 09:56`) |
| Наличие отзыва | `.CountReviews__CommentsSection-sc-n7wfgq-0` внутри `.order-card-client__top-container` |
| Цена отклика | `childNodes[1]` внутри `.order-card-price__container` |
| Ранг отклика | текст `.order-card-additional-info__container` по regex `ваш отклик (\d+)-й` |

Результат — массив из двух строк:

```js
[
  [дата(ДД.ММ.ГГ), "ГГ.ММ.ДД название", url, ранг - 1, "", отзыв (true/false), ""],
  [дата(ДД.ММ.ГГ), название, url, "Профи", "", "", "", "", цена]
]
```

Первая строка уходит как `APPS_SCRIPT_GET` → лист «Отклик», вторая — как `APPS_SCRIPT_GET_RESPONSE` → лист «Заказ».
...
- **Хрупкий парсинг**: селекторы — «скрамблированные» CSS-классы (`-sc-19y55e6-0`, `order-card-*`), которые могут меняться при релизах Профи.ру.
- **Потенциальный `null`**: `parseOrderDateTime` может вернуть `null` (сменился формат даты) → `getOrderCreatedDate` упадёт.
- **`NaN` в ранге**: если ранг не найден, `responseRank - 1` даст `NaN`, который уйдёт в URL как `"NaN"`.
...
- [ ] Устойчивые селекторы (по атрибутам/тексту, не по хэшированным классам)
- [ ] Защита парсера от `null`/`NaN`
